...
class MyVeritProofPostprocessCallback : public ProofNodeUpdaterCallback
{
 public:
  MyVeritProofPostprocessCallback(ProofNodeManager* pnm);
  ~MyVeritProofPostprocessCallback() {}
  /** Should proof pn be updated?
   *
   * @param pn the proof node that maybe should be updated
   * @param continueUpdate indicates whether we should continue recursively updating pn
...
class VeritProofPostprocess
{
 ...
 private:
  ...
  MyVeritProofPostprocessCallback d_cb0;
  std::shared_ptr<VeritProofPostprocessCallback> d_cb;
