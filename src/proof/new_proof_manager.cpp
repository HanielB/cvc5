...
NewProofManager::NewProofManager(context::Context* context, ProofFormat format)
    : d_context(context),
      d_satProof(NULL),
      d_cnfProof(NULL),
      d_theoryProof(NULL),
...
CoreSatProof* NewProofManager::getSatProof() {
  Assert (currentPM()->d_satProof);
  return currentPM()->d_satProof;
}

CnfProof* NewProofManager::getCnfProof() {
  Assert (currentPM()->d_cnfProof);
  return currentPM()->d_cnfProof;
}

TheoryProofEngine* NewProofManager::getTheoryProofEngine() {
  Assert (currentPM()->d_theoryProof != NULL);
  return currentPM()->d_theoryProof;
}

UFProof* NewProofManager::getUfProof() {
  Assert (options::proof());
  TheoryProof* pf = getTheoryProofEngine()->getTheoryProof(theory::THEORY_UF);
  return (UFProof*)pf;
}

proof::ResolutionBitVectorProof* NewProofManager::getBitVectorProof()
{
  Assert (options::proof());
...
void NewProofManager::initSatProof(Minisat::Solver* solver) {
  Assert (currentPM()->d_satProof == NULL);
  Assert(currentPM()->d_format == LFSC);
  currentPM()->d_satProof = new CoreSatProof(solver, d_context, "");
}

void NewProofManager::initCnfProof(prop::CnfStream* cnfStream,
                                context::Context* ctx) {
...
void NewProofManager::initTheoryProofEngine() {
  Assert (currentPM()->d_theoryProof == NULL);
  Assert (currentPM()->d_format == LFSC);
  currentPM()->d_theoryProof = new LFSCTheoryProofEngine();
}

std::string NewProofManager::getInputClauseName(ClauseId id,
                                             const std::string& prefix) {
  return append(prefix+".pb", id);
}

std::string NewProofManager::getLemmaClauseName(ClauseId id,
                                             const std::string& prefix) {
  return append(prefix+".lemc", id);
}

std::string NewProofManager::getLemmaName(ClauseId id, const std::string& prefix) {
  return append(prefix+"lem", id);
}

std::string NewProofManager::getLearntClauseName(ClauseId id,
                                              const std::string& prefix) {
  return append(prefix+".cl", id);
}
std::string NewProofManager::getVarName(prop::SatVariable var,
                                     const std::string& prefix) {
  return append(prefix+".v", var);
}
std::string NewProofManager::getAtomName(prop::SatVariable var,
                                      const std::string& prefix) {
  return append(prefix+".a", var);
}
std::string NewProofManager::getLitName(prop::SatLiteral lit,
                                     const std::string& prefix) {
  return append(prefix+".l", lit.toInt());
}

std::string NewProofManager::getPreprocessedAssertionName(Node node,
                                                       const std::string& prefix) {
...
std::string NewProofManager::getAssertionName(Node node,
                                           const std::string& prefix) {
  return append(prefix+".A", node.getId());
}
std::string NewProofManager::getInputFormulaName(const Expr& expr) {
  return currentPM()->d_inputFormulaToName[expr];
}

std::string NewProofManager::getAtomName(TNode atom,
                                      const std::string& prefix) {
...
std::string NewProofManager::getLitName(TNode lit,
                                     const std::string& prefix) {
...
bool NewProofManager::hasLitName(TNode lit) {
  return currentPM()->d_cnfProof->hasLiteral(lit);
}

std::string NewProofManager::sanitize(TNode node) {
...
void NewProofManager::traceDeps(TNode n, ExprSet* coreAssertions) {
...
void NewProofManager::traceDeps(TNode n, CDExprSet* coreAssertions) {
  Debug("cores") << "trace deps " << n << std::endl;
  if ((n.isConst() && n == NodeManager::currentNM()->mkConst<bool>(true)) ||
      (n.getKind() == kind::NOT && n[0] == NodeManager::currentNM()->mkConst<bool>(false))) {
    return;
...
void NewProofManager::traceUnsatCore() {
...
bool NewProofManager::unsatCoreAvailable() const {
  return d_satProof->derivedEmptyClause();
}

std::vector<Expr> NewProofManager::extractUnsatCore() {
...
void NewProofManager::constructSatProof() {
  if (!d_satProof->proofConstructed()) {
    d_satProof->constructProof();
  }
}

void NewProofManager::getLemmasInUnsatCore(theory::TheoryId theory, std::vector<Node> &lemmas) {
...
std::set<Node> NewProofManager::satClauseToNodeSet(prop::SatClause* clause) {
  std::set<Node> result;
...
Node NewProofManager::getWeakestImplicantInUnsatCore(Node lemma) {
  Assert(PROOF_ON(), "Cannot compute unsat core when proofs are off");
  Assert(unsatCoreAvailable(), "Cannot get unsat core at this time. Mabye the input is SAT?" );
...
void NewProofManager::addAssertion(Expr formula) {
...
void NewProofManager::addDependence(TNode n, TNode dep) {
  if(dep != n) {
...
void NewProofManager::addUnsatCore(Expr formula) {
  Assert (d_inputCoreFormulas.find(formula) != d_inputCoreFormulas.end());
  d_outputCoreFormulas.insert(formula);
}

void NewProofManager::addAssertionFilter(const Node& node, const std::string& rewritten) {
...
void LFSCProof::toStream(std::ostream& out) const
{
  TimerStat::CodeTimer proofProductionTimer(
      *NewProofManager::currentPM()->getProofProductionTime());
  ...
  NewProofManager::getBitVectorProof()->calculateAtomsInBitblastingProof();
  ProofLetMap globalLetMap;
  if (options::lfscLetification()) {
    NewProofManager::currentPM()->printGlobalLetMap(atoms, globalLetMap, out, paren);
  }
  ...
  if (options::bitblastMode() == theory::bv::BITBLAST_MODE_EAGER && NewProofManager::getBitVectorProof()) {
    NewProofManager::getBitVectorProof()->printEmptyClauseProof(out, paren);
  } else {
                                            ...
                                            ProofLetMap& globalLetMap) const
{
  ...
  if (options::fewerPreprocessingHoles()) {
    ...
    for (; it != end; ++it) {
      ...
      if (!NewProofManager::currentPM()->have_input_assertion((*it).toExpr())) {
        ...
        if (((*it).isConst() && *it == NodeManager::currentNM()->mkConst<bool>(true)) ||
          ...
          NewProofManager::currentPM()->traceDeps(*it, &inputAssertions);

          Debug("pf::pm") << "Original assertions for " << *it << " are: " << std::endl;

          NewProofManager::assertions_iterator assertionIt;
          ...
          if (inputAssertions.size() == 0) {
            ...
            inputAssertion = *(NewProofManager::currentPM()->begin_assertions());
          } else {
        ...
        if (!NewProofManager::currentPM()->have_input_assertion(inputAssertion)) {
        ...
        Debug("pf::pm") << "Original assertion for " << *it
                        ...
                        << inputAssertion
                        << ", AKA "
                        << NewProofManager::currentPM()->getInputFormulaName(inputAssertion)
                        << std::endl;

        NewProofManager::currentPM()->getTheoryProofEngine()->printTheoryTerm(inputAssertion, os, globalLetMap);
        os << " ";
        NewProofManager::currentPM()->printTrustedTerm(*it, os, globalLetMap);
        os << "))";
        os << "(\\ "<< NewProofManager::getPreprocessedAssertionName(*it, "") << "\n";
        paren << "))";
        ...
        rewritten << NewProofManager::currentPM()->getInputFormulaName(inputAssertion);
        rewritten << ") (iff_elim_1 _ _ ";
        rewritten << NewProofManager::getPreprocessedAssertionName(*it, "");
        rewritten << "))";

        NewProofManager::currentPM()->addAssertionFilter(*it, rewritten.str());
      }
    }
  } else {
    for (; it != end; ++it) {
...
void LFSCProof::checkUnrewrittenAssertion(const NodeSet& rewrites) const
{
  ...
  for (rewrite = rewrites.begin(); rewrite != rewrites.end(); ++rewrite) {
    ...
    if (NewProofManager::currentPM()->have_input_assertion((*rewrite).toExpr())) {
      Assert(NewProofManager::currentPM()->have_input_assertion((*rewrite).toExpr()));
      Debug("pf::pm") << "LFSCProof::checkUnrewrittenAssertion: this assertion was NOT rewritten!" << std::endl
                      << "\tAdding filter: "
                      << NewProofManager::getPreprocessedAssertionName(*rewrite, "")
                      << " --> "
                      << NewProofManager::currentPM()->getInputFormulaName((*rewrite).toExpr())
                      << std::endl;
      NewProofManager::currentPM()->addAssertionFilter(*rewrite,
        NewProofManager::currentPM()->getInputFormulaName((*rewrite).toExpr()));
    } else {
...
bool NewProofManager::hasOp(TNode n) const {
  return d_bops.find(n) != d_bops.end();
}

Node NewProofManager::lookupOp(TNode n) const {
  std::map<Node, Node>::const_iterator i = d_bops.find(n);
  Assert(i != d_bops.end());
  return (*i).second;
}

Node NewProofManager::mkOp(TNode n) {
...
bool NewProofManager::wasPrinted(const Type& type) const {
  return d_printedTypes.find(type) != d_printedTypes.end();
}

void NewProofManager::markPrinted(const Type& type) {
  d_printedTypes.insert(type);
}

void NewProofManager::addRewriteFilter(const std::string &original, const std::string &substitute) {
  d_rewriteFilters[original] = substitute;
}

bool NewProofManager::haveRewriteFilter(TNode lit) {
...
void NewProofManager::clearRewriteFilters() {
...
void NewProofManager::registerRewrite(unsigned ruleId, Node original, Node result){
  Assert (currentPM()->d_theoryProof != NULL);
  currentPM()->d_rewriteLog.push_back(RewriteLogEntry(ruleId, original, result));
}

void NewProofManager::clearRewriteLog() {
  Assert (currentPM()->d_theoryProof != NULL);
  currentPM()->d_rewriteLog.clear();
}

std::vector<RewriteLogEntry> NewProofManager::getRewriteLog() {
  return currentPM()->d_rewriteLog;
}

void NewProofManager::dumpRewriteLog() const {
...
void NewProofManager::printGlobalLetMap(std::set<Node>& atoms,
                                     ProofLetMap& letMap,
                                     std::ostream& out,
                                     std::ostringstream& paren) {
  Bindings letOrder;
  ...
  const std::set<Node>* additionalAtoms = NewProofManager::getBitVectorProof()->getAtomsInBitblastingProof();
...
void NewProofManager::ensureLiteral(Node node) {
  d_cnfProof->ensureLiteral(node);
}
void NewProofManager::printTrustedTerm(Node term,
                                    std::ostream& os,
                                    ProofLetMap& globalLetMap)
{
  TheoryProofEngine* tpe = NewProofManager::currentPM()->getTheoryProofEngine();
...
NewProofManager::NewProofManagerStatistics::~ProofManagerStatistics()
{
