...
// either p has an expression, or just the name or it already has the kind
// (which is builtin)
Expr Tptp::parseOpToExpr(ParseOp& p)
{
  ...
  {
    Type t = p.d_type == getExprManager()->booleanType() ? p.d_type : d_unsorted;
...
// p may contain just the string of the operator or its builtin kind
