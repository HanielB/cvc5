...
static void resolve(ConstraintCPVec& buf,
                    ConstraintP c,
                    const ConstraintCPVec& pos,
                    const ConstraintCPVec& neg)
{
...
bool TheoryArithPrivate::isProofEnabled() const { return d_pnm != nullptr; }
...
bool TheoryArithPrivate::AssertLower(ConstraintP constraint)
{
  ...
  else
  {
    ...
    if (vc.hasDisequality())
    {
      ...
      if (diseq->isTrue())
      {
        const ConstraintP ub = d_constraintDatabase.ensureConstraint(
            const_cast<ValueCollection&>(vc), UpperBound);
  ...
  d_currentPropagationList.push_back(
...
bool TheoryArithPrivate::AssertUpper(ConstraintP constraint)
{
  ...
  else if (cmpToLB > 0)
  {
    ...
    if (vc.hasDisequality())
    {
      ...
      if (diseq->isTrue())
      {
        const ConstraintP lb = d_constraintDatabase.ensureConstraint(
            const_cast<ValueCollection&>(vc), LowerBound);
...
bool TheoryArithPrivate::AssertDisequality(ConstraintP constraint)
{
  ...
  ArithVar x_i = constraint->getVariable();
  const DeltaRational& c_i = constraint->getValue();
  Trace("arith") << "AssertDisequality(" << x_i << " " << c_i << ")"
                 << std::endl;
  ...
  if (vc.hasLowerBound())
  {
    ...
    if (lb->isTrue())
    {
      const ConstraintP ub = d_constraintDatabase.ensureConstraint(
          const_cast<ValueCollection&>(vc), UpperBound);
      Assert(!ub->isTrue());
      Trace("arith::eq") << "propagate UpperBound " << constraint << lb << ub
                         << endl;
  ...
  if (vc.hasUpperBound())
  {
    ...
    if (ub->isTrue())
    {
      ...
      Trace("arith::eq") << "propagate LowerBound " << constraint << lb << ub
                         << endl;
...
void TheoryArithPrivate::preRegisterTerm(TNode n)
{
  Trace("arith::preregister")
      << "begin arith::preRegisterTerm(" << n << ")" << endl;
  ...
  Trace("arith::preregister")
      << "end arith::preRegisterTerm(" << n << ")" << endl;
}
...
ArithVar TheoryArithPrivate::requestArithVar(TNode x,
                                             bool aux,
                                             CVC5_UNUSED bool internal)
{
...
void TheoryArithPrivate::asVectors(const Polynomial& p,
...
TrustNode TheoryArithPrivate::dioCutting()
{
  ...
  else
  {
    ...
    Trace("arith::dio::ex")
        << "dioCutting found the plane: " << plane.getNode() << endl;
    Trace("arith::dio::ex") << "resulting in the cut: " << lemma << endl;
    Trace("arith::dio::ex") << "rewritten " << rewrittenLemma << endl;
    Trace("arith::dio") << "dioCutting found the plane: " << plane.getNode()
                        << endl;
...
Node TheoryArithPrivate::callDioSolver()
{
  while (!d_constantIntegerVariables.empty())
  {
    ...
    if (eq.isBoolean())
    {
      ...
      // This should be handled by the normal form earlier in the case of
      // equality
    ...
    {
      Trace("dio::push") << "dio::push " << v << " " << eq.getNode()
                         << " with reason " << orig << endl;
...
ConstraintP TheoryArithPrivate::constraintFromFactQueue(TNode assertion)
{
  ...
  if (constraint == NullConstraint)
  {
    ...
    {
      Trace("arith::nf") << "getting non-nf assertion " << assertion << " |-> "
                         << reAssertion << endl;
      Assert(constraint != NullConstraint);
  ...
  if (!constraint->hasProof())
  {
    Trace("arith::constraint")
        << "marking as constraint as self explaining " << endl;
...
bool TheoryArithPrivate::assertionCases(ConstraintP constraint)
{
  ...
  switch (constraint->getType())
  {
    case UpperBound:
      if (isInteger(x_i) && constraint->isStrictUpperBound())
      {
        ...
        if (!floorConstraint->isTrue())
        {
          ...
          {
            Trace("arith::intbound")
                << "literal, before: " << constraint->getLiteral() << std::endl;
            Trace("arith::intbound")
                << "constraint, after: " << floorConstraint << std::endl;
          }
        ...
        if (!ceilingConstraint->isTrue())
        {
          ...
          if (TraceIsOn("arith::intbound"))
          {
            Trace("arith::intbound")
                << "literal, before: " << constraint->getLiteral() << std::endl;
            Trace("arith::intbound")
                << "constraint, after: " << ceilingConstraint << std::endl;
          }
    ...
    case Equality: return AssertEquality(constraint);
    case Disequality: return AssertDisequality(constraint);
...
void TheoryArithPrivate::outputConflicts()
{
  ...
  if (!d_blackBoxConflict.get().isNull())
  {
    ...
    Trace("arith::conflict") << "black box conflict" << bb << endl;
    if (TraceIsOn("arith::normalize::external"))
    {
...
bool TheoryArithPrivate::attemptSolveInteger(Theory::Effort effortLevel,
                                             bool emmmittedLemmaOrSplit)
{
  ...
  Trace("approx") << "attemptSolveInteger " << d_qflraStatus << " "
                  ...
                  << d_lastContextIntegerAttempted << " " << level << endl;

  if (d_qflraStatus == Result::UNSAT)
  {
    return false;
  }
  if (emmmittedLemmaOrSplit)
  {
    return false;
  }
...
std::pair<ConstraintP, ArithVar> TheoryArithPrivate::replayGetConstraint(
    const DenseMap<Rational>& lhs,
    Kind k,
    const Rational& rhs,
    CVC5_UNUSED bool branch)
{
...
ConstraintCP TheoryArithPrivate::vectorToIntHoleConflict(
    const ConstraintCPVec& conflict)
{
  ...
  // This can select negBack multiple times so we need to test if negBack has a
  // proof.
  if (negBack->hasProof())
  {
...
void TheoryArithPrivate::intHoleConflictToVector(ConstraintCP conflicting,
...
void TheoryArithPrivate::tryBranchCut(ApproximateSimplex* approx,
                                      int nid,
                                      BranchCutInfo& bci)
{
  Assert(conflictQueueEmpty());
...
void TheoryArithPrivate::subsumption(std::vector<ConstraintCPVec>& confs) const
{
  ...
  Trace("arith::subsumption")
      << "subsumed " << subsumed << "/" << checks << endl;
}

std::vector<ConstraintCPVec> TheoryArithPrivate::replayLogRec(
    ApproximateSimplex* approx, int nid, ConstraintP bc, int depth)
{
...
void TheoryArithPrivate::importSolution(
    const ApproximateSimplex::Solution& solution)
{
  if (TraceIsOn("arith::importSolution"))
  {
    Trace("arith::importSolution")
        << "importSolution before " << d_qflraStatus << endl;
  ...
  {
    Trace("arith::importSolution")
        << "importSolution after " << d_qflraStatus << endl;
    d_partialModel.printEntireModel(Trace("arith::importSolution"));
  }
}
...
bool TheoryArithPrivate::solveRealRelaxation(Theory::Effort effortLevel)
{
  ...
  if (d_qflraStatus == Result::UNKNOWN && useApprox && safeToCallApprox())
  {
    ...
    ApproximateSimplex* approxSolver =
        ApproximateSimplex::mkApproximateSimplexSolver(
    ...
    switch (relaxRes)
    {
      ...
      case LinExhausted:
        ...
        break;
      case LinUnknown:
      default: ++d_statistics.d_relaxOthers; break;
    }
    delete approxSolver;
  }
...
bool TheoryArithPrivate::postCheck(Theory::Effort effortLevel)
{
  ...
  Trace("arith::ems") << "ems: " << emmittedConflictOrSplit << "pre realRelax"
                      << endl;
  ...
  Trace("arith::ems") << "ems: " << emmittedConflictOrSplit << "post realRelax"
                      << endl;
  ...
  Trace("arith::ems") << "ems: " << emmittedConflictOrSplit << "pre approx cuts"
                      << endl;
  ...
  Trace("arith::ems") << "ems: " << emmittedConflictOrSplit << "post unate"
                      << endl;
  ...
  Trace("arith::ems") << "ems: " << emmittedConflictOrSplit << "pos splitting"
                      << endl;

  Trace("arith") << "integer? "
                 << " conf/split " << emmittedConflictOrSplit << " fulleffort "
                 << Theory::fullEffort(effortLevel) << endl;
      ...
      && !hasIntegerModel())
  {
    ...
    if (!emmittedConflictOrSplit && options().arith.arithDioSolver)
    {
      ...
      if (possibleConflict != Node::null())
      {
        revertOutOfConflict();
        Trace("arith::conflict")
            << "dio conflict   " << possibleConflict << endl;
    ...
    ArithVar x) const
{
  ...
  Trace("integers") << "integers: assignment to [[" << d_partialModel.asNode(x)
                    << "]] is " << r << "[" << i << "]" << endl;
...
std::vector<ArithVar> TheoryArithPrivate::cutAllBounded() const
{
  ...
  if (options().arith.doCutAllBounded && max > 0)
  {
    for (ArithVar iter = 0; iter != max; ++iter)
    {
      ...
      if (isIntegerInput(iter) && !d_cutInContext.contains(iter)
          && d_partialModel.hasUpperBound(iter)
          && d_partialModel.hasLowerBound(iter) && !d.isIntegral())
      {
...
bool TheoryArithPrivate::splitDisequalities()
{
  ...
  while (!d_diseqQueue.empty())
  {
    ...
    else
    {
      ...
      {
        Trace("arith::eq") << "save" << front << ": " << lhsValue
                           << " != " << rhsValue << endl;
...
TrustNode TheoryArithPrivate::explain(TNode n)
{
  ...
  if (c != NullConstraint)
  {
    ...
    Trace("arith::explain")
        << "constraint explanation" << n << ":" << exp << endl;
  }
           ...
           != d_assertionsThatDoNotMatchTheirLiterals.end())
  {
    ...
    if (!c->isAssumption())
    {
      exp = c->externalExplainForPropagation(n);
      Trace("arith::explain")
          << "assertions explanation" << n << ":" << exp << endl;
    }
  }
  else
  {
...
void TheoryArithPrivate::propagate()
{
  ...
  while (d_constraintDatabase.hasMorePropagations())
  {
    ...
    {
      Trace("arith::prop") << "already asserted to the theory "
                           << c->getLiteral() << endl;
    }
  }
...
DeltaRational TheoryArithPrivate::getDeltaValue(TNode term) const
{
  ...
  switch (Kind kind = term.getKind())
  {
    ...
    {  // 2+ args
      ...
      for (TNode::iterator i = term.begin(), iend = term.end(); i != iend; ++i)
      {
    ...
    default: throw ModelException(term, "No model assignment.");
  }
}
...
Rational TheoryArithPrivate::deltaValueForTotalOrder() const
{
  ...
  if (relevantDeltaValues.size() >= 2)
  {
    ...
    std::set<DeltaRational>::const_iterator iter_end =
        relevantDeltaValues.end();
    ...
    std::map<Node, Node>& arithModelIllTyped)
{
  ...
  for (var_iterator vi = var_begin(), vend = var_end(); vi != vend; ++vi)
  {
    ...
    if (!isAuxiliaryVariable(v))
    {
      ...
      {
        Trace("arith::collectModelInfo")
            << "Skipping m->assertEquality(" << term << ", true)" << endl;
      }
    }
  }
...
bool TheoryArithPrivate::entireStateIsConsistent(const string& s)
{
  ...
  for (var_iterator vi = var_begin(), vend = var_end(); vi != vend; ++vi)
  {
    ...
    else if (d_partialModel.isInteger(var)
             && !d_partialModel.integralAssignment(var))
    {
      d_partialModel.printModel(var);
      warning() << s << ":" << "Assignment is not integer for integer variable "
                << var << d_partialModel.asNode(var);
      if (d_tableau.isBasic(var))
      {
        warning() << " (basic)";
      }
...
bool TheoryArithPrivate::unenqueuedVariablesAreConsistent()
{
  ...
  for (var_iterator vi = var_begin(), vend = var_end(); vi != vend; ++vi)
  {
    ...
    if (!d_partialModel.assignmentIsConsistent(var))
    {
      ...
      {
        d_partialModel.printModel(var);
        warning() << "Unenqueued var is not consistent for " << var
                  << d_partialModel.asNode(var);
        if (d_tableau.isBasic(var))
        {
          warning() << " (basic)";
      ...
      {
        d_partialModel.printModel(var);
        warning() << "Initial var is not consistent for " << var
                  << d_partialModel.asNode(var);
        if (d_tableau.isBasic(var))
        {
          warning() << " (basic)";
...
bool TheoryArithPrivate::propagateCandidateBound(ArithVar basic,
                                                 bool upperBound)
{
  ...
  if ((upperBound && d_partialModel.strictlyLessThanUpperBound(basic, bound))
      || (!upperBound
          && d_partialModel.strictlyGreaterThanLowerBound(basic, bound)))
  {
    // TODO: "Policy point"
    ...
    ConstraintP bestImplied =
        d_constraintDatabase.getBestImpliedBound(basic, t, bound);
    ...
    if (bestImplied != NullConstraint)
    {
      ...
      Trace("arith::prop") << "arith::prop" << basic << " "
                           << assertedToTheTheory << " " << canBePropagated
                           << " " << hasProof << endl;
      ...
      {
        Trace("arith::prop")
            << "failed " << basic << " " << bound << " " << assertedToTheTheory
...
{
  /* Four criteria must be met for progagation on a variable to happen using a
   * row: 0: A new bound has to have been added to the row. 1: The
   * hasBoundsCount for the row must be "full" or be full minus one variable
   *    (This is O(1) to check, but requires book keeping.)
   * 2: The current assignment must be strictly smaller/greater than the current
   * bound. assign(x) < upper(x) (This is O(1) to compute.) 3: There is a bound
   * that is strictly smaller/greater than the current assignment. assign(x) < c
   * for some x <= c literal (This is O(log n) to compute.) 4: The implied bound
   * on x is strictly smaller/greater than the current bound. (This is O(n) to
   * compute.)
   */
...
bool TheoryArithPrivate::propagateMightSucceed(ArithVar v, bool ub) const
{
  ...
  if (hasSlack)
  {
    ...
    ConstraintP strongestPossible =
        d_constraintDatabase.getBestImpliedBound(v, t, a);
...
bool TheoryArithPrivate::attemptSingleton(RowIndex ridx, bool rowUp)
{
  ...
  Trace("arith::prop") << "  " << rowUp << " " << v << " " << coeff << " "
                       << vUp << endl;
...
bool TheoryArithPrivate::tryToPropagate(
    RowIndex ridx, bool rowUp, ArithVar v, bool vUb, const DeltaRational& bound)
{
...
bool TheoryArithPrivate::rowImplicationCanBeApplied(RowIndex ridx,
                                                    bool rowUp,
                                                    ConstraintP implied)
{
  ...
  Trace("arith::prop") << "arith::prop" << v << " " << assertedToTheTheory
                       << " " << canBePropagated << " " << hasProof << endl;
  ...
  if (TraceIsOn("arith::prop"))
  {
    Trace("arith::prop") << "failed " << v << " " << assertedToTheTheory << " "
                         << canBePropagated << " " << hasProof << " " << implied
                         << endl;
...
std::pair<bool, Node> TheoryArithPrivate::entailmentCheck(TNode lit)
{
  ...
  Trace("arith::entailCheck")
      << "TheoryArithPrivate::entailmentCheck(" << lit << ")" << endl;
  ...
  DeltaRational sep;
  bool successful =
      decomposeLiteral(lit, k, primDir, lm, lp, rm, rp, dm, dp, sep);
  if (!successful)
  {
  ...
  std::pair<Node, DeltaRational> bestPrimLeft, bestNegPrimRight, bestPrimDiff,
      tmp;
  std::pair<Node, DeltaRational> bestSecLeft, bestNegSecRight, bestSecDiff;
  bestPrimLeft.first = Node::null();
  bestNegPrimRight.first = Node::null();
  bestPrimDiff.first = Node::null();
  bestSecLeft.first = Node::null();
  bestNegSecRight.first = Node::null();
  bestSecDiff.first = Node::null();
  ...
  for (alg = params.begin(), alg_end = params.end(); alg != alg_end; ++alg)
  {
    ...
    Trace("arith::entailCheck")
        << "entailmentCheck trying "
        << (inferbounds::Algorithms)ibalg.getAlgorithm() << endl;
    switch (ibalg.getAlgorithm())
    {
      case inferbounds::None: break;
      case inferbounds::Lookup:
      case inferbounds::RowSum:
      {
        typedef void (TheoryArithPrivate::*EntailmentCheckFunc)(
            std::pair<Node, DeltaRational>&, int, TNode) const;
    ...
    if (!bestPrimLeft.first.isNull() && !bestNegPrimRight.first.isNull())
    {
      ...
      tmp.second =
          ((bestPrimLeft.second * lm) - (bestNegPrimRight.second * rm)) / dm;
    ...
    if (secDir != 0 && !bestSecLeft.first.isNull()
        && !bestNegSecRight.first.isNull())
    {
      ...
      tmp.second =
          ((bestSecLeft.second * lm) - (bestNegSecRight.second * rm)) / dm;
...
void TheoryArithPrivate::setToMin(int sgn,
 ...
 */
bool TheoryArithPrivate::decomposeLiteral(Node lit,
                                          Kind& k,
                                          int& dir,
                                          Rational& lm,
                                          Node& lp,
                                          ...
                                          DeltaRational& sep)
{
  ...
  Trace("arith::decomp") << "arith::decomp " << lit << "(" << normKind << "*"
                         << dir << ")" << endl
                         << "  left:" << lc << " + " << lm << "*(" << lp
                         << ") : " << left << endl
                         << "  right:" << rc << " + " << rm << "*(" << rp
 ...
 */
void TheoryArithPrivate::entailmentCheckBoundLookup(
    std::pair<Node, DeltaRational>& tmp, int sgn, TNode tp) const
{
  tmp.first = Node::null();
  if (sgn == 0)
  {
  ...
  else if (d_partialModel.hasArithVar(tp))
  {
    ...
    ConstraintP c = (sgn > 0) ? d_partialModel.getUpperBoundConstraint(v)
...
void TheoryArithPrivate::entailmentCheckRowSum(
    std::pair<Node, DeltaRational>& tmp, int sgn, TNode tp) const
{
  tmp.first = Node::null();
  if (sgn == 0)
  ...
  for (Polynomial::iterator i = p.begin(), iend = p.end(); i != iend; ++i)
  {
    ...
    if (d_partialModel.hasArithVar(x))
    {
      ...
      ConstraintP c = (dir > 0) ? d_partialModel.getUpperBoundConstraint(v)
                                : d_partialModel.getLowerBoundConstraint(v);
      if (c != NullConstraint)
      {
