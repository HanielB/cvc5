...
{
  Trace("bv-rewrite") << "RewriteRule<BBPlusNeg>(" << node << ")" << std::endl;
  NodeManager *nm = NodeManager::currentNM();
  std::vector<Node> children;
  unsigned neg_count = 0;
  for (unsigned i = 0; i < node.getNumChildren(); ++i)
...
{
  Trace("bv-rewrite") << "RewriteRule<UltPlusOne>(" << node << ")" << std::endl;
  NodeManager *nm = NodeManager::currentNM();
  TNode x = node[0];
  TNode y1 = node[1];
...
{
  Trace("bv-rewrite") << "RewriteRule<IsPowerOfTwo>(" << node << ")"
                      << std::endl;
  NodeManager *nm = NodeManager::currentNM();
