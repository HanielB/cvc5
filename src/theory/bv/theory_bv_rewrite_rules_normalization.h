...
{
  Trace("bv-rewrite") << "RewriteRule<PlusCombineLikeTerms>(" << node << ")"
                      << std::endl;
...
{
  Trace("bv-rewrite") << "RewriteRule<NegPlus>(" << node << ")" << std::endl;
  NodeManager *nm = NodeManager::currentNM();
  std::vector<Node> children;
  for (unsigned i = 0; i < node[0].getNumChildren(); ++i)
  {
...
{
  Trace("bv-rewrite") << "RewriteRule<NormalizeEqPlusNeg>(" << node << ")"
                      << std::endl;
